...
local lexer_grammar = re.compile([[
   tks <- hash? %s* {| (ignore / {| {:i: {} :} token |} )* |}
   ...
   token <- {:tk: keyword ![A-Za-z0-9_] :}   {:kind: () -> 'keyword' :}
          / {:tk: string :}     {:kind: () -> 'string' :}
          / {:tk: symbol1 :}
          / {:tk: op :}         {:kind: () -> 'op' :}
          / {:tk: symbol :}
          / {:tk: identifier :} {:kind: () -> 'identifier' :}
          / {:tk: number :}     {:kind: () -> 'number' :}
   ...
   escape <- "\" ( [abfnrtvz"'\]
                 ...
                 / hexescape
                 / utfescape )
   decimalescape <- digit digit digit / digit digit / digit -- TODO: fail if number > 255
   hexescape <- 'x' hexdigit
   utfescape <- 'u{' hexdigit+ '}'

   identifier <- [a-zA-Z_][a-zA-Z_0-9]*

   number <- hexfloat / hex / float / int
   int <- digit+
   hex <- "0" [xX] hexdigit+
   digit <- [0-9]
   float <- '.' digit* exponentpart?
          / ( digit+ ( '.' digit* exponentpart?
                     / exponentpart ))
   exponentpart <- [eE] [+-]? digit+
   hexdigit <- [0-9a-fA-F]
   hexfloat <- "0" [xX] ( '.' hexdigit+ hexexponentpart?
                        / ( hexdigit+ ( '.' hexdigit* hexexponentpart?
                                      / hexexponentpart )))
...
function tl.lex(input)
   local tokens = lpeg.match(lexer_grammar, input)
   local ys = { 1 }
   local n = 2
