...
| --- | --- | --- | --- | --- |
| `-l --preload`  | `preload_modules` | `{string}` | `build` `check` `gen` `run` | Specify a list of modules that are to be considered by Teal to be pre-loaded into the Lua execution environment. See [below](#preloaded-modules) for details. |
...
Some customized Lua environments pre-load some definitions into the Lua VM
space, for example, setting global variables. An example of an environment
which presents this behavior is <a href="https://love2d.org">Love2D</a>,
which pre-loads a `love` global table containing its API. This global is
just "there", and code written for that environment assumes it is available,
even if you don't load it with `require`.

To make the Teal compiler aware of such pre-loaded definitions, you can
specify them in a definition file (a Teal module ending with a `.d.tl`
extension), and tell the compiler to pre-load it into its own type checking
environment, using the `--preload` flag in the CLI or the `preload_modules`
array in tlconfig.lua.

Note that using `preload_modules` is _not_ a replacement for using `require()`
to load modules in general! This configuration is used to preload into the
compiler type definitons that are already preloaded into the Lua VM
(such as the aforementioned `love` global in Love2D applications). it does
not work as a replacement for `require()`, because it only affects
compile-time checking, and not the Lua code generated by Teal or its execution.
