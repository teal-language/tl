...
| --- | --- | --- | --- | --- |
| `--preload`  | `preload_modules` | `{string}` | `build` `check` `gen` `run` | Specify a list of modules (typically `.d.tl` definition files) to predefine types into the initial global environment. See [below](#global-environment-definitions) for details. |
...
Some customized Lua environments preload some modules into the Lua VM
space, setting global variables. An example of an environment
...
To make the Teal compiler aware of such predefined globals, you can
specify them in a definition file (a Teal module ending with a `.d.tl`
extension), and tell the compiler to predefine it into its own type checking
environment, using the `--preload` flag in the CLI or the `preload_modules`
array in tlconfig.lua.

Note that using `preload_modules` is _not_ a replacement for using
`require()`! This option does not load modules for execution, it only loads
definitions into the compiler, which are meant to be used with modules which
are already preloaded into the Lua VM (such as the aforementioned `love`
global in Love2D applications). It only affects compile-time checking,
and not the Lua code generated by Teal or its execution.
