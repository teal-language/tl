...
local function read_table_item(ps, i, n)
   ...
   elseif ps.tokens[i].kind == "identifier" then
      ...
      elseif ps.tokens[i + 1].tk == ":" then
         ...
         i, node.itemtype = read_type(try_ps, i)
         if node.itemtype and ps.tokens[i].tk == "=" then
            i = verify_tk(try_ps, i, "=")
            i, node[2] = read_table_value(try_ps, i)
            if node[2] then
...
local function read_function_type(ps, i)
   ...
   i, typeargs = read_typeargs_if_any(ps, i)
   if ps.tokens[i].tk == "(" then
      i, typ.args, typ.maybe_method, typ.min_arity = read_argument_type_list(ps, i)
      i, typ.rets = read_return_types(ps, i)
   else
...
local function read_simple_type_or_nominal(ps, i)
   ...
   if ps.tokens[i].tk == "<" then
      i, typ.typevals = read_anglebracket_list(ps, i, read_type)
   end
   return i, typ
end
...
local function read_function_args_rets_body(ps, i, node)
   ...
   i, node.typeargs = read_typeargs_if_any(ps, i)

   i, node[3] = read_argument_list(ps, i)
   i, node.rets = read_return_types(ps, i)
...
do
   ...
   reader.operator = function(node, arity, op)
   ...
   local function failstore(ps, tkop, e1)
      return { f = ps.filename, y = tkop.y, x = tkop.x, kind = "paren", [1] = e1, failstore = true }
   end
   ...
   local function P(ps, i)
      ...
      while true do
         ...
         elseif tkop.tk == "as" or tkop.tk == "is" then
            ...
            if ps.tokens[i].tk == "(" then
               i, cast.casttype = read_type_list(ps, i, "casttype")
            else
               i, cast.casttype = read_type(ps, i)
            end
...
local function read_function(ps, i, fk)
   local orig_i = i
   i = verify_tk(ps, i, "function")
   local fn = new_block(ps, i - 1, "global_function")
   local names = {}
   ...
   if #names > 1 then
      ...
      for i2 = 2, #names - 1 do
         local dot = reader.operator(names[i2], 2, ".")
         names[i2].kind = "identifier"
         owner = { f = ps.filename, y = names[i2].y, x = names[i2].x, kind = "op", op = dot, [1] = owner, [2] = names[i2] }
      end
      fn[2] = owner
   end
   fn[1] = names[#names]
   ...
   i = read_function_args_rets_body(ps, i, fn)
   if fn.is_method and fn[3] then
      table.insert(fn[3], 1, { f = ps.filename, x = selfx, y = selfy, tk = "self", kind = "identifier", is_self = true })
...
local function read_if_block(ps, i, n, node, is_else)
...
local function read_if(ps, i)
   ...
   node[1] = {}
   i, node = read_if_block(ps, i, 1, node)
   if not node then
      return i
   end
   local n = 2
   while ps.tokens[i].tk == "elseif" do
      i, node = read_if_block(ps, i, n, node)
      if not node then
         return i
      end
      n = n + 1
   ...
   if ps.tokens[i].tk == "else" then
      i, node = read_if_block(ps, i, n, node, true)
      if not node then
         return i
      end
   end
...
local function read_nested_type(ps, i, def, tn)
   local istart = i
   i = i + 1
   local iv = i
...
local function read_macroexp(ps, istart, iargs)
   ...
   if ps.tokens[istart + 1].tk == "<" then
      i, node.typeargs = read_anglebracket_list(ps, istart + 1, read_typearg)
   else
      i = iargs
   end
...
local function read_where_clause(ps, i, def)
   ...
   node[1][1].tk = "self"
   node[1][1].argtype = new_type(ps, i, "self");
   (node[1][1].argtype).display_type = def
...
local function read_array_interface_type(ps, i, def)
...
read_record_body = function(ps, i, def)
   ...
   if ps.tokens[i].tk == "{" then
      local atype
      i, atype = read_array_interface_type(ps, i, def)
      if atype then
   ...
   if ps.tokens[i].tk == "is" then
      ...
      if ps.tokens[i].tk == "{" then
         ...
         if ps.tokens[i].tk == "," then
            i = i + 1
            i, def.interface_list = read_trying_list(ps, i, {}, read_interface_name)
         else
            def.interface_list = {}
         end
         if atype then
   ...
   if ps.tokens[i].tk == "where" then
      ...
      i, where_macroexp = read_where_clause(ps, i, def)
   ...
   while not (ps.tokens[i].kind == "$EOF$" or ps.tokens[i].tk == "end") do
      ...
      elseif read_type_body_fns[tn] and ps.tokens[i + 1].tk ~= ":" then
         if def.typename == "interface" and tn == "record" then
            ...
            i = read_nested_type(ps, i, def, tn)
         end
      else
         ...
         if ps.tokens[i].tk == ":" then
            ...
            if ps.tokens[i].tk == "=" and ps.tokens[i + 1].tk == "macroexp" then
               ...
               if tt.typename == "function" then
                  i, tt.macroexp = read_macroexp(ps, i + 1, i + 2)
               else
...
do
   ...
   read_call_or_assignment = function(ps, i)
      ...
      if reader.node_is_funcall(exp) or exp.failstore then
         return i, exp
      end
...
local function read_variable_declarations(ps, i, node_name)
   ...
   i, asgn.decltuple = read_type_list(ps, i, "decltuple")
...
local function read_local(ps, i)
   ...
   elseif read_type_body_fns[tn] and ps.tokens[i + 2].kind == "identifier" then
      return read_type_constructor(ps, i, "local_type", tn)
   end
...
local function read_global(ps, i)
   ...
   local tn = ntk
   if ntk == "function" then
      return read_function(ps, i + 1, "global")
   elseif ntk == "type" and ps.tokens[i + 2].kind == "identifier" then
      return read_type_declaration(ps, i + 2, "global_type")
   elseif read_type_body_fns[tn] and ps.tokens[i + 2].kind == "identifier" then
      return read_type_constructor(ps, i, "global_type", tn)
