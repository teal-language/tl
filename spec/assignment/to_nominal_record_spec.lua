...
describe("assignment to nominal record", function()
   ...
   it("fails if table has extra fields", function()
      ...
      assert.match("in local declaration: x: unknown field bla", errors[1].err, 1, true)
   end)
   ...
   it("fails if mismatch", function()
      ...
      local _, ast = tl.parse_program(tokens)
      local errors = tl.type_check(ast)
      assert.match("in local declaration: x: got number, expected Node", errors[1].err, 1, true)
   end)
end)
