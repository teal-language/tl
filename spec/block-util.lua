...
local function check(lax, code, unknowns, gen_target, lang)
   return function()
      ...
      local blocks, syntax_errors = reader.read(code, "foo.lua", lang)
      assert.same({}, syntax_errors, "Blocks was not expected to have syntax errors")
      local ast, syntax_errors = block_parser.parse(blocks, "foo.lua", lang)
