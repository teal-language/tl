...
describe("[]", function()
   describe("on records", function()
      ...
      it("ok without declaration if key is enum and all keys map to the same type", function()
         ...
         errors = tl.type_check(ast)
         assert.same(1, #errors)
         assert.match("cannot index, not all enum values map to record fields of the same type", errors[1].err, 1, true)
         -- fail as expected
         local tokens = tl.lex([[
            local Keys = enum
               "foo"
         ...
         errors = tl.type_check(ast)
         assert.same(1, #errors)
         assert.match("enum value 'oops' is not a field", errors[1].err, 1, true)
      end)
