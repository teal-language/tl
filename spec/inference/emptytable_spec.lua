...
describe("empty table without type annotation", function()
   it("has its type determined by its first use", function()
      ...
      assert.match("cannot index something that is not a record: {number}", errors[1].err, 1, true)
   end)
   ...
   it("cannot be reassigned to a non-table", function()
      ...
      local _, ast = tl.parse_program(tokens)
      local errors = tl.type_check(ast, false, "test.lua")
      assert.match("assigning number to a variable declared with {}", errors[1].err, 1, true)
   end)
   ...
   it("preserves provenance information", function()
      ...
      local _, ast = tl.parse_program(tokens)
      local errors = tl.type_check(ast, false, "test.lua")
      assert.match("cannot index something that is not a record: {number} (inferred at test.lua:5:", errors[1].err, 1, true)
   end)
end)
